  ...
  reducers: {
    ...
    applyPromoCode: (state, action: PayloadAction<PromoCode>) => {
      ...
      if (action.payload.type === 'percentage') {
        state.promoDiscount = Math.round((state.adjustedTotalPrice * action.payload.discount) / 100);
      } else {
...
export const { addToCart, removeCartItem, remove, applyPromoCode, removePromoCode } = cartsSlice.actions;
